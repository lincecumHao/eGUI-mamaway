...
define([
  ...
  function downloadEmptyFile(format_file) {
    try {
      ...
      if (_error_message.length != 0) {
        ...
        //////////////////////////////////////////////////////////////////////
        //字軌檔清單+發票資料
        var _empty_invoice_ary = searchEmptyAssignLog(
          _business_no,
          _year_month,
          _assign_log_status_ary,
          _format_code_ary,
          _egui_electric,
          _egui_manual
        )
    ...
    try {
      ...
      if (empty_invoice_ary != null) {
        ...
        for (var i = 0; i < empty_invoice_ary.length; i++) {
          ...
          //TODO
          if (
            checkIsSeqNumber(_start_no, _pre_end_invoice_number) == false ||
  ...
  function checkIsSeqNumber(start_invoice_number, pre_end_invoice_number) {
    ...
    try {
      if (pre_end_invoice_number.length != 0) {
        ...
        if (
          parseFloat(start_invoice_number) == parseFloat(pre_end_invoice_number)
        )
          _result = true
      }
    } catch (e) {
  ...
  //空白字軌
  function searchEmptyAssignLog(
    business_no,
    year_month,
    assign_log_status_ary,
    format_code_ary,
    egui_electric,
    egui_manual
  ) {
    ...
    try {
      //空白字軌清單
      var _empty_assign_log_ary = searchAssignLog(
        business_no,
        year_month,
        assign_log_status_ary,
        format_code_ary
      ...
      var _error_invoice_ary = searchVoucherList(
        business_no,
        year_month,
        _upload_status,
        format_code_ary,
  ...
  function searchAssignLog(
    ...
    try {
      ...
      _mySearch.run().each(function (result) {
        ...
        if (
          parseFloat(_assignlog_lastinvnumber) != parseFloat(_assignlog_endno)
        ) {
          var _json_ary = {
  ...
  function uploadEmptyXml(business_no, year_month, empty_json_obj_ary) {
    try {
      ...
      var _e0402_xml_ary = migxmlutility.getE0402Xml(
        business_no,
        year_month,
        empty_json_obj_ary,
        _temp_e0402_xml
      ...
      for (var i = 0; i < _e0402_xml_ary.length; i++) {
        ...
        var _api_result = sendToGateWebE0402API(
          _business_no,
          _year_month,
          _invoice_track,
          _invoice_type,
  ...
  function sendToGateWebE0402API(
    business_no,
    year_month,
    invoice_track,
    invoice_type,
