...
define([
  ...
  function deleteSelected() {
    try {
      ...
      var _checkJsonObject = validateDeleteTask()
      var _checkFlag = _checkJsonObject.checkflag
      var _message = _checkJsonObject.message
  ...
  function deleteVoucher(voucher_list_id) {
    //1.delete details
    //2.delete main
    //3.delete apply
    try {
      ...
      var _detailSearch = search.create({
        ...
        columns: [
          ...
          search.createColumn({ name: 'custrecord_gw_ns_document_type' }), //INVOICE
      ...
      //_filterArray.push(['custrecord_gw_dtl_apply_internal_id', search.Operator.ALLOF, _internalIdAry]);
      ...
      if (_invoice_id_ary.length != 0) {
        for (var i = 0; i < _invoice_id_ary.length; i++) {
          ...
          _record.setValue({
            fieldId: _invoce_control_field_id,
            value: _invoce_control_field_value,
          })
          try {
            _record.save()
          } catch (e) {
