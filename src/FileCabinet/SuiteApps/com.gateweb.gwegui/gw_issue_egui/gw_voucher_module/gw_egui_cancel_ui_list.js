...
define([
  ...
  function searchVoucherList(
    ...
    for (var i = 0; i < _pagedData.pageRanges.length; i++) {
      ...
      _currentPage.data.forEach(function (result) {
        ...
        //if (_is_printed_paper==true || _is_printed_pdf==true)_is_printed='是';

        var _is_printed_pdf_text = ''
        var _is_printed_paper_text = ''
        if (_is_printed_paper == true) _is_printed_paper_text = '是'
        if (_is_printed_pdf == true) _is_printed_pdf_text = '是'
        ...
        if (_indexId != _id) {
          ...
          subListObj.setSublistValue({
            id: 'customer_need_upload_egui_mig',
            line: _index,
            value: _need_upload_egui_mig,
          })
          ...
          subListObj.setSublistValue({
            id: 'customer_voucher_isprinted_paper',
            line: _index,
            value: stringutility.trimOrAppendBlank(_is_printed_paper_text),
          })
          subListObj.setSublistValue({
            id: 'customer_voucher_isprinted_pdf',
            line: _index,
            value: stringutility.trimOrAppendBlank(_is_printed_pdf_text),
          })
  ...
  function createInvoiceSubList(form) {
    ...
    //20210128 walter modify 是否上傳
    var _needUploadMigField = _sublist.addField({
      id: 'customer_need_upload_egui_mig',
      label: 'Need Upoload MIg',
      type: serverWidget.FieldType.TEXT,
    })
    _needUploadMigField.updateDisplayType({
      displayType: serverWidget.FieldDisplayType.HIDDEN,
    })
    /////////////////////////////////////////////////////////////////////////////
    //處理顯示欄位
  ...
  function onRequest(context) {
    ...
    form.addButton({
      ...
      label: '作廢發票',
      functionName: 'submitCancelProcess("SINGLE-EGUI-SCHEDULE")',
    })
    form.addButton({
    ...
    form.addButton({
      ...
      label: '重傳發票E-Mail',
      functionName: 'submitEmailProcess("EGUI")',
    })
    form.addButton({
