define(['../../library/ramda.min'], function (ramda) {
  /**
   * Module Description...
   *
   * @type {Object} module-name
   *
   * @copyright 2021 Gateweb
   * @author Sean Lin <sean.hyl@gmail.com>
   *
   * @NApiVersion 2.0
   * @NModuleScope Public

   */
  var exports = {}
  var recordId = 'customrecord_gw_mig_type'
  var fieldConfig = {
    name: {
      id: 'name',
      sourceField: '',
      outputField: 'name',
    },
    custrecord_gw_mt_egui_type: {
      id: 'custrecord_gw_mt_egui_type',
      sourceField: '',
      outputField: 'documentType',
    },
    custrecord_gw_mt_bus_tran_type: {
      id: 'custrecord_gw_mt_bus_tran_type',
      sourceField: '',
      outputField: 'businessTranType',
    },
    custrecord_gw_mt_mig_type: {
      id: 'custrecord_gw_mt_mig_type',
      sourceField: '',
      outputField: 'migType',
    },
    custrecord_gw_mt_xml_filename: {
      id: 'custrecord_gw_mt_xml_filename',
      sourceField: '',
      outputField: 'xmlFileName',
    },
    custrecord_gw_mt_action_mode: {
      id: 'custrecord_gw_mt_action_mode',
      sourceField: '',
      outputField: 'action',
    },
  }

  var fieldInputMapping = ramda.reduce(
    function (result, fieldId) {
      var fieldObj = fieldConfig[fieldId]
      if (fieldObj.sourceField) {
        result[fieldId] = fieldObj.sourceField
      }
      return result
    },
    {},
    Object.keys(fieldConfig)
  )

  var fieldOutputMapping = ramda.reduce(
    function (result, fieldId) {
      var fieldObj = fieldConfig[fieldId]
      if (fieldObj.outputField) {
        result[fieldObj.id] = fieldObj.outputField
      }
      return result
    },
    {},
    Object.keys(fieldConfig)
  )

  exports.fields = fieldConfig
  exports.allFieldIds = Object.keys(fieldConfig).map(function (key) {
    return key
  })
  exports.fieldOutputMapping = fieldOutputMapping
  exports.fieldInputMapping = fieldInputMapping
  exports.recordId = recordId
  return exports
})
